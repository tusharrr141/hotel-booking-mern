...
const Dashboard = () => {
  const [dasshboardData, setDasshboardData] = useState(dashboardDummyData);
  return (
    <div>
      <Title
        align="left"
      ...
      <div className="flex gap-4 my-8">
        {/* Total Booking */}
        <div className="bg-primary/3 borderborder-primary/10 rounded flex p-4 pr-8">
          <img
      ...
      <div className="w-full max-w-3x1 text-left border border-gray-300 rounded-1g max-h-80 overflow-y-scroll">
        <table className="w-full">
          ...
          <tbody className="text-sm">
            {dasshboardData.bookings.map((item, index) => (
              <tr key={index}>
                ...
                <td className="py-3 px-4 flex border-t border-gray-300">
                  <button
                    className={`py-1 px-3 rounded-full mx-auto text-xs ${
                      item.isPaid
